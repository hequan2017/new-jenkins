...
本期范围:MVP 引擎,Step 类型为 HTTP 回调与本地 Shell 命令,在 GVA 进程内执行;状态与日志通过 SSE 实时推送。
...
- 菜单/权限:通过后台菜单管理手动建,`component` 填 `view/workflow/...vue`,`name` 对应上述 defineOptions name
...
接口(全部 PrivateGroup,`@Security ApiKeyAuth`):
...
- 构建运行实例对 Stage/Step 定义做**快照**(`snapshot_name/snapshot_order`),定义后续被改不影响历史构建记录
- 审批 gate 的 Stage 跑完前置 Step 后,build 置 `running-approval`,等 `approveStage` 唤醒;最后一个 Stage 的 Approval 不阻塞
- 定时触发(schedule)与 Webhook 触发的执行入口预留(`trigger_type` 字段),本期调度器/Webhook 入口未实现
