...
// Snapshot* 在构建启动时从定义拷贝, 之后不受定义变更或删除影响。
type PipelineBuildStage struct {
	...
	SnapshotApproval        bool       `json:"snapshotApproval" form:"snapshotApproval" gorm:"column:snapshot_approval;comment:构建时的审批配置快照"`
	SnapshotContinueOnError bool       `json:"snapshotContinueOnError" form:"snapshotContinueOnError" gorm:"column:snapshot_continue_on_error;comment:构建时的失败继续配置快照"`
	SnapshotParallel        bool       `json:"snapshotParallel" form:"snapshotParallel" gorm:"column:snapshot_parallel;comment:构建时的并行配置快照"`
